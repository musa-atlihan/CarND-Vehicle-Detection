...
To generate the heat maps, a background image (having all values equal to zero) with the same size of the region of the interest is created. Then, whenever there is a detection, the pixel intensities for that detection area of the background image are increased by one. This way, the intensity values increased by the number of overlaps constituting a heat map. Below an example of a heat map is given from a test image.

![Heat map image](./images/heat-map.png)

The bright colors on the heat map are where the most overlaps occur. In order to eliminate the false positives a thresholding is also applied (the chosen value for thresholding is 1) on the heat maps. This eliminates most of the false positives and helps with the accuracy of the results. After the heat map implementation, the islands on a heat map image are detected and labelled just using the `label` function of [scipy](https://www.scipy.org/)'s [ndimage library](https://docs.scipy.org/doc/scipy-0.16.0/reference/generated/scipy.ndimage.measurements.label.html). The resulting labelling for two cars are given in different colors below.
...
The car finding pipeline mentioned above is tested on the project video. The model detects the cars reliably. However, borders of the detection rectangles change too rapidly in each frame. In order to overcome this situation, `groupRectangles` function of the [OpenCV](http://docs.opencv.org/2.4/modules/objdetect/doc/cascade_classification.html#grouprectangles) library is used to cluster and average the rectangles of similar size and similar locations in an array of consecutive results. This lets the detection borders become more consistent. And in addition, helps discard the appearing false positives for short time intervals. However, results still display some false negative detections as it is shown in the example below.
...
Using Hog features, it is possible to reach very high accuracies. And since there are too many overlapping sliding windows to search an image for detection and localization problems, reducing the false positives by reaching low error rates becomes important. However, Extracting HOG features is a computationally expensive process. In this project, a single scale overlapping sliding windows approach on a region of interest with 1280x380 pixels in size took about 20 minutes for a 50 seconds video (about 1 iterations per second) despite computing the whole HOG features at once. Thus, this approach does not seem to offer a sufficient solution for real time applications. 
...
For These reasons, using the power of convolutional neural network models might be a better choice and help improve the results. There are various models which are known to be very fast (using the benefits of GPUs and parallel computing) and state of the art on localization tasks like [Fast R-CNN](https://arxiv.org/abs/1504.08083), [YOLO](https://arxiv.org/abs/1506.02640), [YOLO9000](./https://arxiv.org/abs/1612.08242), etc. Further implementations with these architectures may help implementing fast classifiers (for many classes) which is an important factor for real time self-driving car applications.